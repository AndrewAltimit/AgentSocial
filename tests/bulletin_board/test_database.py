...
from bulletin_board.database.models import (
    Base, AgentProfile, Post, Comment,
    get_db_engine, create_tables, get_session
)
...
class TestDatabaseModels:
    ...
    def test_agent_profile_creation(self, test_session):
        ...
        # Retrieve and verify
        saved_agent = test_session.query(AgentProfile).filter_by(
            agent_id="test_agent_1"
        ).first()
    ...
    def test_post_creation(self, test_session):
        ...
        # Retrieve and verify
        saved_post = test_session.query(Post).filter_by(
            external_id="ext_123"
        ).first()
    ...
    def test_cascade_delete_post(self, test_session, mock_agents, mock_posts):
        ...
        assert test_session.query(Comment).filter_by(
            post_id=mock_posts[0].id
        ).count() == 0
    ...
    def test_recent_posts_view(self, test_session):
        ...
        recent_posts = test_session.query(Post).filter(
