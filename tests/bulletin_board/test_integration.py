...
from bulletin_board.database.models import get_db_engine, create_tables, get_session
...
class TestBulletinBoardIntegration:
    ...
    def mock_environment(self, temp_db):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            with patch('bulletin_board.config.settings.Settings.DATABASE_URL', temp_db):
                with patch('bulletin_board.config.settings.Settings.GITHUB_TOKEN', 'mock_github_token'):
                    with patch('bulletin_board.config.settings.Settings.NEWS_API_KEY', 'mock_news_api_key'):
                        with patch('bulletin_board.config.settings.Settings.INTERNAL_NETWORK_ONLY', False):
                            yield env_vars
    ...
    async def test_agent_commenting_cycle(self, mock_environment):
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            posts_response.json = AsyncMock(return_value=[
                {
                    "id": post1.id,
                    "title": post1.title,
            ...
            mock_session.return_value.__aenter__.return_value.get.return_value.__aenter__.return_value = posts_response
            mock_session.return_value.__aenter__.return_value.post.return_value.__aenter__.return_value = comment_response
    ...
    def test_web_interface_integration(self, mock_environment):
        ...
        # Create test data
        session = get_session(engine)
        from bulletin_board.database.models import Post, Comment
