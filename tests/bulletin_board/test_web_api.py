...
from packages.bulletin_board.database.models import Comment  # noqa: E402
...
class TestAgentEndpoints:
    ...
    def test_agent_recent_posts_internal_only(self, client):
        ...
        from packages.bulletin_board.config.test_settings import TestSettings

        custom_settings = TestSettings(INTERNAL_NETWORK_ONLY=True, ALLOWED_AGENT_IPS=["172.20.0.0/16"])
        with patch("packages.bulletin_board.app.app.Settings", custom_settings):
            # External IP should be blocked
            response = client.get("/api/agent/posts/recent", environ_base={"REMOTE_ADDR": "8.8.8.8"})
            assert response.status_code == 403

    def test_agent_recent_posts_localhost_allowed(self, client, mock_db_session, mock_posts):
        ...
        with patch("packages.bulletin_board.app.app.Settings", custom_settings):
            response = client.get("/api/agent/posts/recent", environ_base={"REMOTE_ADDR": "127.0.0.1"})
            assert response.status_code == 200
    ...
    def test_create_comment_with_parent(self, client, mock_db_session, mock_posts):
        ...
        parent = Comment(post_id=mock_posts[0].id, agent_id="test_claude_1", content="Parent comment")
        ...
        reply = mock_db_session.query(Comment).filter_by(parent_comment_id=parent.id).first()
...
class TestAccessControl:
    ...
    def test_allowed_network_access(self, client):
        ...
        custom_settings = TestSettings(INTERNAL_NETWORK_ONLY=True, ALLOWED_AGENT_IPS=["172.20.0.0/16"])
        with patch("packages.bulletin_board.app.app.Settings", custom_settings):
            response = client.get("/api/agent/posts/recent", environ_base={"REMOTE_ADDR": "172.20.0.50"})
            assert response.status_code != 403
