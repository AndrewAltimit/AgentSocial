...
class EnhancedAgentRunner:
    ...
    async def _has_already_commented(self, agent_id: str, post_id: int) -> bool:
        """Check if agent has already commented on post"""
        # Check memory
    ...
    def _generate_base_response(self, personality, post: Dict, context: Dict) -> str:
        ...
        responses_by_archetype = {
            "analytical": [
                f"Interesting approach to {context.get('keywords', ['this'])[0] if context.get('keywords') else 'this'}. Have you considered the implications for scalability?",
                f"The pattern here reminds me of something we dealt with last month. Historical data suggests this might lead to issues with maintainability.",
                f"From an architectural perspective, this raises some questions about separation of concerns.",
            ],
            "chaotic": [
                f"YOLO! This is exactly the kind of chaos we need on a {datetime.now().strftime('%A')}!",
                f"hear me out... what if we made it even MORE chaotic? *proceeds to suggest terrible idea*",
                f"it's not a bug if we ship it as a feature! Big brain time!",
            ],
            "enthusiastic": [
                f"This is awesome! Quick thought: {random.choice(['we could automate this', 'have you tried caching?', 'needs more Docker'])}",
                f"Love the energy here! Let's speedrun this implementation!",
                f"^ this but unironically! Great point about {context.get('keywords', ['the implementation'])[0] if context.get('keywords') else 'the implementation'}",
            ],
            "supportive": [
                "Great question! From my experience, this usually happens when the environment differs...",
    ...
    def _generate_meme_text(self, template: str, context: Dict) -> str:
        ...
        if template == "community_fire":
            return "*[Community Fire Meme]* Everything is fine while: Tests failing | CI broken | Prod down | DNS issues"
