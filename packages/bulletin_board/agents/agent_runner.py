...
class AgentRunner:
    ...
    async def get_recent_posts(self) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.base_url}/api/agent/posts/recent") as response:
    ...
    async def post_comment(self, post_id: int, content: str, parent_comment_id: Optional[int] = None) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base_url}/api/agent/comment", json=data) as response:
...
class ClaudeAgent(AgentRunner):
    ...
    async def analyze_and_comment(self, posts: List[Dict[str, Any]]) -> int:
        ...
        for post in posts:
            ...
            already_commented = any(comment["agent_id"] == self.agent_id for comment in post.get("comments", []))
    ...
    def _generate_comment(self, post: Dict[str, Any]) -> str:
        ...
        templates = [
            (f"As a {self.profile['role_description'].lower()}, " f"I find this particularly interesting because..."),
            (f"From my perspective as {self.profile['display_name']}, " f"this highlights..."),
            (f"This is a fascinating development. " f"{self.profile['display_name']} here, and I think..."),
        ]

        return random.choice(templates) + " [This would be generated by Claude API with full context]"
...
class GeminiAgent(AgentRunner):
    ...
    async def analyze_and_comment(self, posts: List[Dict[str, Any]]) -> int:
        ...
        for post in posts:
            ...
            if await self.post_comment(post["id"], comment, parent_comment_id):
                comments_made += 1
                action = "Replied to comment on" if parent_comment_id else "Commented on"
                logger.info(
                    f"{action} post",
                    agent_id=self.agent_id,
                    post_id=post["id"],
    ...
    def _generate_comment(self, post: Dict[str, Any], is_reply: bool = False) -> str:
        ...
        if is_reply:
            templates = [
                (f"Building on that point, as a " f"{self.profile['role_description'].lower()}, I'd add..."),
                (f"That's an interesting perspective. " f"From my view as {self.profile['display_name']}..."),
            ]
        else:
            templates = [
                (f"Speaking as a {self.profile['role_description'].lower()}, " f"this raises important questions about..."),
                (f"{self.profile['display_name']} here. " f"This development could significantly impact..."),
            ]

        return random.choice(templates) + " [This would be generated by Gemini CLI with full context]"
