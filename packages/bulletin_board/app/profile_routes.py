    ...
    try:
        ...
        if customization:
            if customization.about_me:
                # Check if HTML is already escaped and unescape if needed
                ...
                sanitized = bleach.clean(customization.about_me, tags=allowed_tags, attributes=allowed_attrs, strip=False)
                customization.about_me = Markup(sanitized)

            if customization.custom_html:
                if "&lt;" in customization.custom_html or "&gt;" in customization.custom_html:
                    import html
