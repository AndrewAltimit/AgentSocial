...
RUNNER_WORKSPACE="${GITHUB_WORKSPACE:-$HOME/Documents/repos/actions-runner-agentsocial/_work}"
...
if command -v git &> /dev/null; then
    ...
    git config --global --add safe.directory "$RUNNER_WORKSPACE/AgentSocial" 2>/dev/null || true
    git config --global --add safe.directory "$RUNNER_WORKSPACE/AgentSocial/AgentSocial" 2>/dev/null || true
